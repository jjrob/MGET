...
class MatlabDependency(Dependency):
    ...
    def GetConstraintDescriptionStrings(self):
        return [_('MATLAB Runtime R2026a (which can be freely downloaded from https://www.mathworks.com/products/compiler/matlab-runtime.html) or the full version of MATLAB R2026a')]
    ...
    def FindMatlab(cls, setPath=False, loggingQueue=None):
        ...
        if sys.platform == 'linux':
            ...
            if len(oldLdLibraryPath.strip()) > 0:
                ...
                for dir in oldLdLibraryPath.split(':'):
                    if os.path.isfile(os.path.join(dir, fileToFind)):
                        _LogDebug('Found %s. MATLAB R2026a or MATLAB Runtime R2026a appears to be installed.' % os.path.join(dir, fileToFind))
                        return None
            ...
            for defaultDir in defaultDirs:
                ...
                if os.path.isfile(defaultFile):
                    _LogDebug('%s exists. MATLAB R2026a or MATLAB Runtime R2026a appears to be installed.' % defaultFile)
        ...
        # plaform and need to fail.
            ...
            raise UnsupportedPlatformError(_('This tool rquires MATLAB R2026a or the MATLAB Runtime R2026a, support for accessing MATLAB when running on the %r platform has not been implemented yet. Please contact the developer of this tool for assistance.') % sys.platform)

        # If we fell through to here, we did not find MATLAB.

        raise SoftwareNotInstalledError(_('This tool requires that MATLAB R2026a or the MATLAB Runtime R2026a be installed. The MATLAB Runtime is free and may be downloaded from https://www.mathworks.com/help/compiler/install-the-matlab-runtime.html. Please follow the installation instructions carefully. Version R2026a must be used; other versions will not work. MATLAB Runtime allows multiple versions can be installed at the same time.'))
